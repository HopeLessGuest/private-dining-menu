...
export const DishItem: React.FC<DishItemProps> = ({
  ...
  const categoryName = dish.category && CATEGORY_TRANSLATIONS[dish.category] 
      ? CATEGORY_TRANSLATIONS[dish.category][language] 
      : dish.category;
        ...
        {dish.isCustomBuilder ? (
           ...
           <>
                ...
                <div className="flex flex-wrap gap-1.5 mb-2 mt-1">
                    {/* Locked Featured Tag */}
                    {dish.featured && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 border border-amber-200 text-[10px] font-medium text-amber-800 tracking-wide">
                            <Star size={8} className="fill-amber-700 text-amber-700" /> {t.chefsSpecial}
                        </span>
                    )}
                    {/* Other Tags */}
                    {dish.tags?.map((tag) => (
                        <span key={tag} className="inline-flex items-center px-2 py-0.5 rounded-full bg-slate-100 border border-slate-200 text-[10px] font-medium text-slate-600">
                            {tag}
                        </span>
                    ))}
                </div>
